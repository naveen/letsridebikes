...
Foursquare.prototype.todos = function(callback) {
    this.makeRequest('users/self/todos?sort=recent&limit=200',
                     function(response) { callback(response['response']['todos']['items']) });
};

Foursquare.prototype.userTodos = function(user, callback) {
		this.makeRequest('users/' + user + '/todos?sort=recent&limit=200',
										function(response) { callback(response['response']['todos']['items']) });
};
